...
namespace codicastudio\NovaImpersonate\Http\Controllers;
...
class ImpersonateController extends Controller
{
    ...
    {
        $guardName = $guardName ?? config('admin-impersonate.default_impersonator_guard');
        ...
        if (config('admin-impersonate.leave_before_impersonate') && $this->manager->isImpersonating()) {
            if (config('admin-impersonate.actionable')) {
        ...
        $redirectBack = config('admin-impersonate.redirect_back');

        if ($redirectBack) {
            session()->put('leave_redirect_to', $redirectBack === true ? url()->previous() : $redirectBack);
        }

        return redirect()->to($request->get('redirect_to', config('admin-impersonate.redirect_to')));
    }
